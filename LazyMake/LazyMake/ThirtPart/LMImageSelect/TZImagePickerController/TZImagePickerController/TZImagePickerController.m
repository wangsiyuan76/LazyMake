...
@interface TZImagePickerController () <TOCropViewControllerDelegate> {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    albumPickerVc.ifUsedTZCrop = self.ifUsedTZCrop;
    albumPickerVc.customAspectRatio = self.customAspectRatio;
    @weakify(self);
    albumPickerVc.noTZCropBlock = ^(UIImage *cropImage) {
        @strongify(self);
        if (self.noTZCropBlock) {
            [self dismissViewControllerAnimated:YES completion:nil];
            self.noTZCropBlock(cropImage);
        }
    };
    LMLog(@"======%@",albumPickerVc);
}
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        photoPickerVc.ifSlectChange = self.ifUsedTZCrop;
        @weakify(self);
        photoPickerVc.changeCropBlock = ^(NSArray *models, NSInteger index) {
            @strongify(self);
            if (models.count > index) {
                TZAssetModel *subModel = models[index];
                [MBProgressHUD showMessage:@"照片同步中"];
                [[TZImageManager manager] getPhotoWithAsset:subModel.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                    if (!isDegraded && photo) {
                         [MBProgressHUD hideHUD];
                         dispatch_async(dispatch_get_main_queue(), ^{
                             [MBProgressHUD hideHUD];
                             TOCropViewController  *cropController = [[TOCropViewController alloc] initWithImage:photo];
                             //选择框可以按比例来手动调节  customAspectRatio
                             ...
                             cropController.customAspectRatio = self.customAspectRatio;
                             //显示选择按钮
                        ...
                        if (!photo) {
                            ...
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [MBProgressHUD hideHUD];
                                [MBProgressHUD showError:@"手机内存空间过低"];
                            });
                        }
                    }
                }];
...
@interface TZAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate,TOCropViewControllerDelegate> {
