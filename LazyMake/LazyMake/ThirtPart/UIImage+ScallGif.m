...
+ (NSData *)scallGIFWithData:(NSData *)data scallSize:(CGSize)scallSize succeedBlock:(void (^)(NSData *  _Nonnull resultData))completion{
  if (!data) {
    return nil;
  }
  CGImageSourceRef source = CGImageSourceCreateWithData((__bridge CFDataRef)data, NULL);
  ...
  CFRelease(source);
    if (completion) {
        completion([NSData dataWithContentsOfFile:tempFile]);
    }
