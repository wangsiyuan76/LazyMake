...
#define MAX_COUNT 30                    //当前屏幕显示多少个略缩图

@interface SJVideoCropView ()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
{
...
@property (nonatomic, assign) NSInteger showNum;        //每几秒显示一张图片
@property (nonatomic, strong) UILabel *beginLab;        //开始时间
...
-(void)BaseStting{
        _itemBackgroundColor=[UIColor colorWithHexString:@"#1DABFD"];
        self.clipsToBounds=NO;
        _FivecollectionWidth=(self.width-60);
        _imagesArray = [NSMutableArray array];

        if (_mediaConfig.sourceDuration > MAX_COUNT) {
            _itemWidth=_FivecollectionWidth / MAX_COUNT;
            _itemCount=MAX_COUNT;
            _showNum = _mediaConfig.sourceDuration/MAX_CANON;
        }else{
            _itemCount=_mediaConfig.sourceDuration;
            _itemWidth=_FivecollectionWidth / _mediaConfig.sourceDuration;
        }
    ...
    _imageViewWith = 15;
    _margion=(self.frame.size.width -_mediaConfig.maxDuration*_perSpWith-_imageViewWith*2)/2.0;
...
//    followLayout.itemSize = CGSizeMake(_itemWidth , 60);
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 40, self.width-_margion, 60) collectionViewLayout:followLayout];
...
- (void)setupSubviews {
    _durationLaber=[[UILabel alloc]initWithFrame:CGRectMake(0, 108, self.frame.size.width, 20)];
    _durationLaber.font=[UIFont systemFontOfSize:12];
    _durationLaber.backgroundColor=[UIColor clearColor];
    ...
    _beginLab=[[UILabel alloc]initWithFrame:CGRectMake(20, 6, kScreenWidth, 20)];
    _beginLab.font=[UIFont systemFontOfSize:12];
    _beginLab.backgroundColor=[UIColor clearColor];
    _beginLab.textColor=[UIColor whiteColor];
    _beginLab.textAlignment=NSTextAlignmentLeft;
    [self addSubview:_beginLab];
    
    _playBtn = [[UIButton alloc]initWithFrame:CGRectMake((self.frame.size.width - 40)/2, 0, 40, 40)];
    ...
    _endLab=[[UILabel alloc]initWithFrame:CGRectMake(20, 6, self.frame.size.width-40, 20)];
    _endLab.font=[UIFont systemFontOfSize:12];
    ...
    _imageViewRight.frame = CGRectMake(self.frame.size.width - _imageViewWith-_margion, _collectionView.y, _imageViewWith, _collectionView.height);
...
- (void)loadThumbnailData {
    _durationLaber.text=[NSString stringWithFormat:@"已选择%@秒",[NSString stringWithFormat:@"%.0f",_mediaConfig.endTime - _mediaConfig.startTime]];
    _beginLab.text= [NSString stringWithFormat:@"%.2d:%.2d",(int)_mediaConfig.startTime/60,(int)_mediaConfig.startTime%60];
    _endLab.text= [NSString stringWithFormat:@"%.2d:%.2d",(int)_mediaConfig.endTime/60,(int)_mediaConfig.endTime%60];
    self.timeData = _mediaConfig.endTime - _mediaConfig.startTime;
    self.beginTime = (int)_mediaConfig.startTime;
    self.endTime = (int)_mediaConfig.endTime;
    ...
    [self.imageGenerator generateCGImagesAsynchronouslyForTimes:array completionHandler:^(CMTime requestedTime, CGImageRef  _Nullable image, CMTime actualTime, AVAssetImageGeneratorResult result, NSError * _Nullable error) {
        ...
        if (result == AVAssetImageGeneratorSucceeded) {
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                ...
                index++;
                LMLog(@"===indexindex=======%d ====== %d",index,array.count);
                if (index == weakSelf.mediaConfig.sourceDuration || index == array.count) {
                    if ([weakSelf.delegate respondsToSelector:@selector(addCoverAllImage:)]) {
                        [weakSelf.delegate addCoverAllImage:weakSelf.imagesArray];
                    }
                }
            });
        }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake(_FivecollectionWidth / _imagesArray.count, 60);
}
...
- (void)clickPlayBtn:(UIButton *)btn{
    ...
    if ([_delegate respondsToSelector:@selector(clickPlayBtn:)]) {
        [_delegate clickPlayBtn:btn.selected];
    }
}
@end
