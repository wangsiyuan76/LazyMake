...
@property(nonatomic,assign)CGFloat minSpacing;              //中间最小间距
...
-(void)setConfig:(DoubleSliderConfig *)config
{
    ...
    CGFloat radio = self.config.maxValue/self.backgroundLine.width;
    self.minSpacing = (self.config.minInterval-1)/radio;
}
...
-(void)panleftSliderBtn:(UIPanGestureRecognizer *)pan
{
    ...
    if (rightCenter.x - (point.x+leftCenter.x) < self.minSpacing) {
        if (pan.state == UIGestureRecognizerStateEnded) {
            [self leftValueChange:rightCenter.x - self.minSpacing ifEnd:YES];
            [self rightValueChange:rightCenter.x ifEnd:YES];
            if (self.panResponse) {
                self.panResponse(self.currentLeftValue, self.currentRightValue, self.leftButton.center, self.rightButton.center,YES,YES);
            }
        }
        return;
    }else{
        ...
        if (pan.state == UIGestureRecognizerStateChanged) {
            [self tapLeftRefresh:point rightCenter:rightCenter leftCenter:leftCenter ifEnd:NO];
        }
        if (pan.state == UIGestureRecognizerStateEnded) {
            [self tapLeftRefresh:point rightCenter:rightCenter leftCenter:leftCenter ifEnd:YES];
        }
    }
    

}
-(void)tapLeftRefresh:(CGPoint)point rightCenter:(CGPoint)rightCenter leftCenter:(CGPoint)leftCenter ifEnd:(BOOL)ifEnd{
    ...
    self.leftLine.width = self.leftButton.centerX;
    [self rightValueChange:self.rightButton.centerX ifEnd:ifEnd];
    [self leftValueChange:self.leftButton.centerX ifEnd:ifEnd];
    if (self.panResponse) {
        self.panResponse(self.currentLeftValue, self.currentRightValue, self.leftButton.center, self.rightButton.center,ifEnd,YES);
    }
...
-(void)panrightSliderBtn:(UIPanGestureRecognizer *)pan
{
    ...
    if ((rightCenter.x + point.x) - leftCenter.x < self.minSpacing) {
        if (pan.state == UIGestureRecognizerStateEnded) {
            [self leftValueChange:leftCenter.x ifEnd:YES];
            [self rightValueChange:leftCenter.x + self.minSpacing ifEnd:YES];
             if (self.panResponse) {
                 self.panResponse(self.currentLeftValue, self.currentRightValue, self.leftButton.center, self.rightButton.center,YES,NO);
             }
        }
        return;
    }
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        [self tapRightRefresh:point rightCenter:rightCenter leftCenter:leftCenter boolIfEnd:NO];
    }
    if (pan.state == UIGestureRecognizerStateEnded) {
        [self tapRightRefresh:point rightCenter:rightCenter leftCenter:leftCenter boolIfEnd:YES];
    }
}
-(void)tapRightRefresh:(CGPoint)point rightCenter:(CGPoint)rightCenter leftCenter:(CGPoint)leftCenter boolIfEnd:(BOOL)ifEnd{
    ...
    [self rightValueChange:self.rightButton.centerX ifEnd:ifEnd];
    if (self.panResponse) {
        self.panResponse(self.currentLeftValue, self.currentRightValue, self.leftButton.center, self.rightButton.center,ifEnd,NO);
    }
}
//MARK: - 滑块值的改变以及响应
-(void)leftValueChange:(CGFloat)maxX ifEnd:(BOOL)ifEnd{
...
-(void)rightValueChange:(CGFloat)maxX ifEnd:(BOOL)ifEnd{
    CGFloat radio = maxX/self.backgroundLine.width;
...
//    leftButton.layer.borderWidth = 0.5;
    ...
    rightButton.frame = CGRectMake(self.width-10, self.sliderOffsetY+self.height/2-30, 20, 20);
    rightButton.backgroundColor = [UIColor colorWithRed:77/255.0 green:217/255.0 blue:100/255.0 alpha:0.0];
